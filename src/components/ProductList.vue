<template>
  <Table class="p-4 h-full" :items="products">
    <div v-for="product in products" :key="product.id" class="flex flex-col md:flex-row items-center p-4 rounded shadow">
    <span class=" text-lg mr-8 font-bold mb-2 md:mb-0">{{ product.id }}</span>
      <img :src="product.image" alt="Product Image" class="w-20 h-20 flex-1 object-cover mb-2 md:mb-0 md:mr-6" />
      ...
      <button @click="tambahProduct(product)" class="ml-0 md:ml-2  md:w-auto p-2 w-full bg-green-500 text-white rounded hover:bg-green-700 mb-2 md:mb-0">
        <font-awesome-icon icon="cart-plus" class="mr-2" />Add
      </button>
    <button @click="removeProduct(product.id)" class="ml-0 md:ml-2  md:w-auto p-2 w-full bg-red-500 text-white rounded hover:bg-red-700 mb-2 md:mb-0">
