      ...
      <div class="md:mr-4 w-full md:w-1/2 mb-4 md:mb-0">
        ...
        <div class="relative mb-4">
          <input
            ...
            class="block w-full pl-10 pr-4 py-2 mb-4 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md shadow-sm"
          />
        ...
        <div v-if="selectedCustomer" class="mt-4 p-4 bg-gray-50 rounded-lg shadow-inner">
          ...
          <div class="text-gray-700 space-y-3">
            <p class="flex items-center space-x-2">
              <i class="fas fa-user text-green-500"></i> <strong>Name:</strong> <span>{{ selectedCustomer?.customer_name }}</span>
            </p>
            <p class="flex items-center space-x-2">
              <i class="fas fa-envelope text-yellow-500"></i> <strong>Email:</strong> <span>{{ selectedCustomer?.email }}</span>
            </p>
            <p class="flex items-center space-x-2">
              <i class="fas fa-phone text-blue-500"></i> <strong>Phone:</strong> <span>{{ selectedCustomer?.phone }}</span>
            </p>
            <p class="flex items-center space-x-2">
              <i class="fas fa-map-marker-alt text-red-500"></i> <strong>Address:</strong> <span>{{ selectedCustomer?.address }}</span>
            </p>
            <p v-if="selectedCustomer?.company" class="flex items-center space-x-2">
              <i class="fas fa-briefcase text-purple-500"></i> <strong>Company:</strong> <span>{{ selectedCustomer?.company }}</span>
            </p>
            <p v-if="selectedCustomer?.website" class="flex items-center space-x-2">
              <i class="fas fa-globe text-indigo-500"></i> <strong>Website:</strong> <span><a :href="selectedCustomer?.website" class="text-indigo-600" target="_blank">{{ selectedCustomer?.website }}</a></span>
            </p>
          </div>
          <div class="mt-4 flex justify-end space-x-2">
  ...
  setup(props, { emit }) {
    ...
    const updateCustomer = () => {
      if (selectedCustomer.value) {
        ...
        if (index !== -1) {
          customers.value[index] = { ...selectedCustomer.value, customer_name: 'Updated Customer' };
        }
      }
    };
